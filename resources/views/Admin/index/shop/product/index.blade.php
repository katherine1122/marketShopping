            ...
            <form class="am-form-inline" role="form" method="get">
                ...
                <div class="am-form-group">
                    <select data-am-selected="{btnSize: 'sm', maxHeight: 360, searchBox: 1}"
                        ...
                        <optgroup label="{{$category->name}}" value="{{$category->parent_id}}">
                            @foreach($category->children as $c)
                            <option value="{{$c->id}}"
                            @if($c->id == Request::input('$category->id')) selected @endif>
                                {{$c->name}}
                            </option>
                            @endforeach
                        </optgroup>
                    ...
                    <select data-am-selected="{btnSize: 'sm', maxHeight: 360, searchBox: 1}"
                        ...
                        @foreach($brands as $brand)
                            <option value="{{$brand->id}}"
                                @if($brand->id == Request::input('$brand->id')) @endif>
                                {{$brand->name}}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="am-form-group">
                    <div class="am-btn-group" data-am-button="">
                        ...
                        @if(Request::input('is_top') == 1) am-active @endif ">
                            <input type="checkbox" name="is_top" value="1"
                                    @if(Request::input('is_top')==1) checked @endif> 置顶
                        </label>
                        <label class="am-btn am-btn-default am-btn-sm am-radius
                        @if(Request::input('is_recommend')==1) am-active @endif">
                            <input type="checkbox" name="is_recommend" value="1"
                                   @if(Request::input('is_recommend')==1) checked @endif> 推荐
                        </label>
                        <label class="am-btn am-btn-default am-btn-sm am-radius
                        @if(Request::input('is_hot')==1) am-active @endif">
                            <input type="checkbox" name="is_hot" value="1"
                                   @if(Request::input('is_hot')==1) checked @endif> 热销
                        </label>
                        <label class="am-btn am-btn-default am-btn-sm am-radius
                        @if(Request::input('is_new')==1) am-active @endif">
                            <input type="checkbox" name="is_new" value="1"
                                   @if(Request::input('is_new')==1) checked @endif> 新品
                        </label>
                    </div>
                </div>
                ...
                <div class="am-form-group">
                    <select data-am-selected="{btnSize: 'sm'}" name="is_onsale" id="">
                        <option value="-1"
                                @if(Request::input('is_onsale') == '-1') selected @endif>上架状态</option>
                        <option value="1"
                                @if(Request::input('is_onsale')=='1') selected @endif>上架</option>
                        <option value="0"
                                @if(Request::input('is_onsale')=='0') selected @endif>下架</option>
                    </select>
                </div>

                <div class="am-form-group">
                    <input type="text" id="created_at" placeholder="选择时间日期" name="created_at"
                           value="{{Request::input('create_at')}}" class="am-form-field am-input-sm"/>
                </div>
            ...
            <form class="am-form" id="form">
                <table class="am-table am-table-striped am-table-hover table-main">
                    ...
                    <tr data-id="{{$product->id}}">
                        ...
                        <td>{{$product->id}}</td>
                        <td>
                            <img src="{{$product->thumb}}" alt="" class="thumb" style="width: 40px" height="60px">                                </td>
                        <td>
                            {{$product->name}}
                        </td>
                        ...
                        <td class="am-hide-sm-only">
                            <input type="text" name="stock" class="am-input-sm" id="stock"
                        ...
                        <td>
                            <div class="am-btn-toolbar">
                                <div class="am-btn-group am-btn-group-xs">
                                    ...
                                    <a class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only" href="{{route('shop.product.destroy',$product->id)}}" data-method="delete" data-token="{{csrf_token()}}" data-confirm="确认删除吗?">
                                        <span class="am-icon-trash-o"></span> 删除
                                    </a>
                                </div>
                            </div>
